...
def _convert_depthwise_weights(conv2d_layer: Conv2D, depthwise_layer: DepthwiseConv2D):
    ...
    if not weights:
        return weights
    kernel, *rest = weights
    # Grouped Conv2D: (kH, kW, in_channels/groups, filters) -> DepthwiseConv2D: (kH, kW, in_channels, depth_multiplier)
    kernel = np.transpose(kernel, (0, 1, 3, 2))
    return [kernel, *rest]


def clone_depthwise_conv(layer):
    """Map grouped Conv2D depthwise layers to DepthwiseConv2D for GPU training."""
