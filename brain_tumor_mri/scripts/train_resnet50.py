"""Train ResNet50 on brain tumor MRI (binary: NO_TUMOR vs TUMOR)."""
import argparse
import json
import os
import shutil
...
DEFAULT_VAL_ACC_CAP = 0.99


class ValAccuracyCap(tf.keras.callbacks.Callback):
    """Stop fine-tune before val accuracy saturates at 1.0; keep best weights below cap."""

    def __init__(
        self,
        cap: float = DEFAULT_VAL_ACC_CAP,
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if (
            val_acc < self.cap
            and val_acc >= self.min_val_acc
            and val_acc > self.best_val
        ):
            ...
            if self.checkpoint_path is not None:
                ...
                print(
                    f"Saved Phase 2 best weights (val_acc={val_acc:.4f}) "
                    f"to {self.checkpoint_path}"
                )
        if val_acc >= self.cap:
            print(
                f"Val accuracy {val_acc:.4f} reached cap {self.cap:.2f}; "
                "restoring best weights below cap and stopping Phase 2."
            )
            if self.best_weights is not None:
                self.model.set_weights(self.best_weights)
            else:
                print("No valid Phase 2 checkpoint below cap; caller should keep Phase 1.")
...
def make_test_generator(test_df, shuffle=False, rescale_input=True):
    ...
    return datagen.flow_from_dataframe(dataframe=test_df, **_generator_kwargs(shuffle))


def make_generators(train_df, val_df, test_df, shuffle_train=True, rescale_input=True):
    datagen = ImageDataGenerator(rescale=1.0 / 255 if rescale_input else None)
    train_gen = datagen.flow_from_dataframe(dataframe=train_df, **_generator_kwargs(shuffle_train))
    val_gen = datagen.flow_from_dataframe(dataframe=val_df, **_generator_kwargs(False))
...
def fit_model(
    ...
    callbacks.append(
        EarlyStopping(
            patience=patience,
            restore_best_weights=use_checkpoint,
            monitor=monitor,
            mode=mode,
        )
    )
...
def configure_devices():
    ...
    if gpus:
        try:
            ...
            print(f"Could not restrict to first GPU: {exc}")
...
def main():
    parser = argparse.ArgumentParser(description="Train ResNet50 on brain tumor MRI.")
    ...
    phase1_path = models_dir / "cnn_4_phase1.h5"
    phase2_path = models_dir / "cnn_4_phase2_best.h5"
    ...
    print(
        f"Phase 2: fine-tune all layers "
        f"(patience={args.finetune_patience}, val_acc_cap={args.val_acc_cap})"
    )
    fit_model(
        model,
        train_gen,
    ...
    model.save(models_dir / "cnn_4_final.h5", include_optimizer=False)
    ...
    for split_name, (df_split, gen_split) in split_map.items():
        ...
        plot_confusion_matrix(
            cm,
            f"Brain Tumor MRI - {split_name.upper()}",
    ...
    preds_df.to_csv(args.output_dir / "test_predictions.csv", index=False)

    print(f"\nSaved model to: {models_dir / 'cnn_4_final.h5'}")
