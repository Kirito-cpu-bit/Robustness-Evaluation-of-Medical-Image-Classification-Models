...
def pgd_attack_batch(
    ...
    adv_images = tf.clip_by_value(images + random_start, clip_min, clip_max)

    for _ in range(steps):
        with tf.GradientTape() as tape:
            tape.watch(adv_images)
            predictions = model(adv_images * scale, training=False)
...
def compute_metrics(y_true, y_prob, epsilon, attack_name="PGD", steps=DEFAULT_STEPS, clean_probs=None):
