...
The complete experimental codebase used to produce the results in this dissertation is available for anonymous peer review at:
\begin{center}
  \url{https://anonymous.4open.science/r/medical-adversarial-robustness-REVIEW}
\end{center}
\noindent
Replace \texttt{REVIEW} with the identifier assigned when uploading the repository to \url{https://anonymous.4open.science/}. The repository contains task-specific pipelines under \path{brain_tumor_mri/} and \path{chest_xray/}, exported LaTeX tables under \path{paper_tables/}, and dissertation figures under \path{thesis/figures/}. A top-level \path{README.md} summarises the workflow; the full developer manual is in \path{docs/DEVELOPER_MANUAL.md} (Appendix~B).

\chapter{Developer Manual}
\label{app:dev-manual}

This appendix summarises how to reproduce the experiments on a new machine. The canonical version with script tables and troubleshooting is maintained in the repository at \path{docs/DEVELOPER_MANUAL.md}.

\section{Environment and dependencies}

Python~3.8--3.10 is recommended. Core dependencies are pinned in \path{requirements.txt}:
TensorFlow~2.12, NumPy, pandas, scikit-learn, Matplotlib, Seaborn, Pillow, and \texttt{huggingface\_hub}. Brain MRI download additionally requires the \texttt{datasets} package. GPU experiments in this project used TensorFlow with the DirectML plugin on Windows~11 (RTX~4060 Laptop GPU, 8~GB VRAM); on Linux, install a CUDA-matched TensorFlow build instead.

\section{Setup on a new server}

\begin{enumerate}
  \item Clone or copy the repository and create a virtual environment.
  \item Install dependencies: \path{pip install -r requirements.txt} plus \path{datasets} and (on Windows) \path{tensorflow-directml-plugin}.
  \item Download data: \path{brain_tumor_mri/run_pipeline.py --steps download} and \path{chest_xray/run_pipeline.py --steps download}.
  \item Fine-tune models: \path{run_pipeline.py --skip-download --steps train-resnet50 train-densenet121 ...}.
  \item Run attacks and fair comparison: \path{--steps fgsm pgd bim deepfool fair}.
  \item Brain MRI defences (optional): \path{scripts/train_adversarial.py} then \path{scripts/evaluate_gaussian_defense.py}.
\end{enumerate}

Scripts import local modules (\path{config}, \path{data_utils}, \path{attack_config}) and should be launched via \path{run_pipeline.py} or with working directory set to the task's \path{scripts/} folder.

\section{Script map (selected)}

\begin{table}[htbp]
  \centering
  \caption{Where to find key functionality in the repository.}
  \label{tab:script-map}
  \small
  \begin{tabular}{p{3.2cm}p{4.2cm}p{5.8cm}}
    \toprule
    Function & Script(s) & Output location \\
    \midrule
    Data download & \path{download_dataset.py} & \path{data/} \\
    Standard training & \path{train_resnet50.py}, \path{train_model.py} & \path{outputs/*/models/*.h5} \\
    FGSM / PGD / BIM & \path{fgsm_attack.py}, \path{pgd_attack.py}, \path{bim_attack.py} & --- \\
    Cross-model comparison & \path{model_*_comparison.py} & \path{outputs/model_comparison/} \\
    Fair robustness & \path{model_fair_robustness_comparison.py} & Summary CSV/Markdown \\
    PGD-AT & \path{train_adversarial.py} & \path{outputs/adversarial/models/} \\
    Gaussian D0--D3 & \path{evaluate_gaussian_defense.py} & \path{outputs/defense/gaussian/} \\
    \bottomrule
  \end{tabular}
\end{table}

\chapter{Representative Code Snippets}
\label{app:snippets}

The listings below are abbreviated excerpts from the repository illustrating the pipeline orchestrator, PGD attack core, fair robustness normalisation, and Gaussian defence conditions. Line numbers refer to the version submitted for review.

\section{Pipeline orchestration (\path{brain_tumor_mri/run_pipeline.py})}

\begin{lstlisting}
STEPS = {
    "download": [PYTHON, "scripts/download_dataset.py", ...],
    "train-resnet50": [PYTHON, "scripts/train_resnet50.py", ...],
    "pgd": [PYTHON, "scripts/model_pgd_comparison.py", ...],
    "fair": [PYTHON, "scripts/model_fair_robustness_comparison.py", ...],
    "train-pgd-at-resnet50": [PYTHON, "scripts/train_adversarial.py", ...],
}
for step in args.steps:
    subprocess.run(STEPS[step] + extra, cwd=SCRIPTS, check=True)
\end{lstlisting}

\section{PGD attack batch (\path{brain_tumor_mri/scripts/pgd_attack.py})}

\begin{lstlisting}
def pgd_attack_batch(model, images, labels, epsilon, alpha, steps):
    labels = tf.reshape(labels, (-1, 1))
    random_start = tf.random.uniform(tf.shape(images), -epsilon, epsilon)
    ...
    for _ in range(steps):
        ...
        adv_images = tf.clip_by_value(adv_images,
            images - epsilon, images + epsilon)
        adv_images = tf.clip_by_value(adv_images, 0.0, 1.0)
    return adv_images
\end{lstlisting}

\section{Fair robustness relative drop (\path{model_fair_robustness_comparison.py})}

\begin{lstlisting}
# rel_drop(m) = (clean(m) - adv(m)) / clean(m)
for metric in ("accuracy", "adv_auc", "f1"):
    clean_val = float(clean[metric])
    adv_val = float(row[metric])
    out[f"{metric}_rel_drop"] = (clean_val - adv_val) / clean_val
\end{lstlisting}

\section{Gaussian defence conditions D0--D3 (\path{evaluate_gaussian_defense.py})}

\begin{lstlisting}
CONDITIONS = {
    "D0": ("standard", blur_clean=False, blur_adv=False),
    "D1": ("standard", blur_clean=False, blur_adv=True),
    "D2": ("standard", blur_clean=True,  blur_adv=True),
    "D3": ("pgd_at",   blur_clean=True,  blur_adv=True),
}
# Attacks generated on unblurred x; blur applied before forward pass.
inputs = maybe_blur(adv_images, spec.blur_adv, sigma)
