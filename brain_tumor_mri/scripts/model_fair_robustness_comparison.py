...
from comparison_common import EPSILONS, MODEL_STYLE, models_title
from config import MODEL_COMPARISON_DIR
...
def process_attack(
    ...
    if not csv_path.exists():
        print(f"Skipping {attack_name}: missing {csv_path}")
        return None
...
def main():
    ...
    combined_parts = []
    summary = {}
    for attack_key, attack_name in [
        ("fgsm", "FGSM"),
        ("bim", "BIM"),
        ("pgd", "PGD"),
        ("deepfool", "DeepFool"),
    ]:
        rel_df = process_attack(args.input_dir, figures_dir, attack_key, attack_name)
        if rel_df is None:
            continue
        sections.append((attack_name, rel_df))
        combined_parts.append(rel_df)
        summary[attack_name] = rel_df.drop(
            columns=["confusion_matrix"], errors="ignore"
        ).to_dict("records")

    if not combined_parts:
        raise FileNotFoundError("No attack comparison CSVs found under input-dir.")
    ...
    write_markdown(sections, md_path)
