...
@tf.function
def fgsm_step(model, images, labels, epsilon, clip_min=0.0, clip_max=1.0):
...
def generate_adversarial_batches(
    ...
    adv_images = []
    eps_t = tf.constant(epsilon, tf.float32)
    for start in range(0, len(images), batch_size):
        ...
        adv_images.append(
            fgsm_step(model, batch_x, batch_y, eps_t, clip_min, clip_max).numpy()
        )
